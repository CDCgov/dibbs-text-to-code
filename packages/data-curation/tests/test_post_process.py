...
class TestApplyDeletionPostProcessing:
    ...
    def test_deletions_in_long_code_string(self):
        code_str = "Myelin associated glycoprotein/Sulfated glucuronic paragloboside IgM Ab [Titer] in Serum by Immunoassay"  # noqa
        fsn = "Myelin associated glycoprotein:Titr:Pt:Ser:SemiQn:IA"  # noqa
        deleted = post_process.apply_deletion_post_processing(
            code_str, fsn=fsn, loinc_enhancements=LOINC_ENHANCEMENTS
        )
        assert deleted == "Myelin associated glycoprotein/Sulfated glucuronic IgM Ab [Titer] Serum by"  # noqa
...
class TestApplyPOCPostProcessing:
    def test_poc(self):
        assert post_process.apply_point_of_care_post_processing(
            "Auto RBC (Bld)"
        ) == "POC Auto RBC (Bld)"
...
class TestPoundSignPosttProcessing:
    def test_no_pound_signs(self):
        input = "Creatinine [Mass/volume] in Serum or Plasma"
        assert post_process.apply_pound_sign_post_processing(
            input, outer_handling_method="drop"
        ) == input
    ...
    def test_multiple_inner_pounds(self):
        ...
        assert dropped == "Automated (Number concentration) platelets [Number/volume] in Blood"  # noqa
...
class TestTruncationPostProcessing:
    ...
    def test_string_over_trunc_limit(self):
        input = "Myelin associated glycoprotein/Sulfated glucuronic paragloboside IgM Ab [Titer] in Serum by Immunoassay"  # noqa
        truncated = post_process.apply_truncation_post_processing(input)
        assert truncated == "Myelin associated glycoprotein/Sulfated glucuronic paragloboside IgM Ab [Titer] in Se"
