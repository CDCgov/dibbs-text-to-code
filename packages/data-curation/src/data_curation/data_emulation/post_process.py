...
from data_curation.data_emulation.loinc_utils import LOINC_PREPOSITIONS, _find_system_modality, _get_component_axis_from_fsn
...
def apply_deletion_post_processing(code_str: str, **kwargs) -> str:
    ...
    if component_idx != -1 and present_component is not None:
        ...
        idxs_to_exclude = list(range(
            len(preceding_words),
            len(preceding_words) + len(present_component.strip().split())
        ))
    ...
    deletion_eligible_idxs = [
        i for i  in range(len(words)) if not i in idxs_to_exclude
    ]
...
def apply_modality_drop_post_processing(code_str: str, **kwargs) -> str:
    ...
    if system_modality is not None:
        dropped = code_str[:system_modality[1]].strip() + " " + \
            code_str[system_modality[2]:].strip()
