...
RERANKER_TRAINING_OUT_FILE = (
    "../../../../data/training_files/prod_emulated_reranker_pairs_no_cn.txt"
)
...
NAME_VARIANTS = ["long_common_name", "short_name", "display_name", "consumer_name"]
LOINC_AXES = ["component", "property", "time", "system", "scale", "method"]
MAX_NUM_AXIS_REPLACEMENT_TRIES = 100
BIG_THREE_LIQUID_MODALITIES = [
    "Serum",
    "Plasma",
    "Blood",
    "Serum or Plasma",
...
SINGLE_WORD_METHOD_MAPPINGS = {"by screen method": "Screening", "by automated count": "Automated"}
...
def build_and_process_ttc_and_heuristics(
    ...
    if _codestring_is_valid_candidate(ttc_build, code_str):
        ...
        ttc_processed = _choose_and_apply_post_processing(
            ttc_build, fsn, system_axis, LOINC_ENHANCEMENTS, BASE_POST_PROCESSING_OPTIONS
        )
        if _codestring_is_valid_candidate(ttc_processed, code_str):
            variations.append(ttc_processed)

    rule_based_example = _choose_and_apply_heuristics(code_str, property_axis, system_axis)
    ...
    heuristic_ex_to_process = _choose_and_apply_heuristics(code_str, property_axis, system_axis)
    heuristic_ex_to_process = _choose_and_apply_post_processing(
        heuristic_ex_to_process, fsn, system_axis, LOINC_ENHANCEMENTS, BASE_POST_PROCESSING_OPTIONS
    )
...
def build_loinc_axis_example(
    ...
    component = _get_component_axis_from_fsn(loinc_code.fully_specified_name)
    if component == "":
        return ""
    ...
    relevant_axes = [
        c
        for c in LOINC_AXES
        if c != "component" and (c != "time" or not skip_time) and (c != "scale" or not skip_scale)
    ]

    for axis in relevant_axes:
        choice_for_axis = _choose_from_loinc_axis(getattr(loinc_code, axis), LOINC_ENHANCEMENTS)
        if choice_for_axis != "":
...
def build_short_name_hyphen_variant(code_str: str) -> str:
    ...
    """
    if "-" in code_str:
        return code_str.split("-", maxsplit=1)[0]
    return ""
...
def build_ttc_enhanced_example(code_str: str) -> str:
    ...
    """
    num_enhancements_to_attempt = 1 + math.ceil(float(len(code_str.split())) / 4.0)
...
def build_vendor_formula_style_example(code_str: str, property_axis: str, system_axis: str) -> str:
    ...
    if bracketed_text is not None:
        ...
        if "presence" in bracketed_text.group(0).lower():
            ...
            result = (
                result[: bracketed_text.span()[0]].strip()
                + " "
    ...
    for nv in NAME_VARIANTS:
        ...
        if nv == "short_name":
            ...
            processed_sn = _choose_and_apply_post_processing(
                sn, fsn, loinc_code.system, LOINC_ENHANCEMENTS, ["dot", "poc", "pound", "modality"]
            )
            ...
            if nv == "long_common_name":
                ...
                vendor_build = build_vendor_formula_style_example(
                    name, loinc_code.property, loinc_code.system
                )
...
def get_modality_variations(code_str: str, system_axis: str) -> list[str]:
    ...
    if modality is not None:
        ...
        variant = (
            apply_syntax_post_processing(modality[0]).strip()
            + " "
    ...
    if modality is not None:
        ...
        if modality[0] in BIG_THREE_LIQUID_MODALITIES:
            swap_candidates = [c for c in BIG_THREE_LIQUID_MODALITIES if c != modality[0]]
        ...
        if system_axis in POC_SYSTEMS:
            variant = (
                code_str[: modality[1]].strip() + " Whole Blood " + code_str[modality[2] :].strip()
            )
            variations.append(variant.strip())
...
def get_q_variations(code_str: str) -> list[str]:
    ...
    if q_group is not None:
        ...
        v1 = code_str[: q_group.span()[0]] + modality + " " + code_str[q_group.span()[1] + 1 :]
...
def _allocate_generated_loincs_to_training_arrays(
    ...
    generated_loincs: list[str],
    search_training_array: list[str],
    ...
    if len(generated_loincs) > 0:
        ...
        if remaining_examples > 0:
            ...
            for i in range(1, 1 + allocate_to_search):
                search_training_array.append((code_string_base_name, generated_loincs[i]))
            if remaining_examples - allocate_to_search > 0:
                for j in range(1 + allocate_to_search, len(generated_loincs)):
                    reranker_training_array.append((code_string_base_name, generated_loincs[j]))
...
if __name__ == "__main__":
    ...
    with open(SNOINC_DATA_FILE, encoding="utf-8") as fp:
        data = fp.readlines()
    data = data[1:]
    ...
    for row in data:
        ...
        (
            ...
            consumer_name,
        ) = r[0], r[1], r[2], r[3], r[4], r[5], r[6], r[7], r[8], r[9], r[10], r[13], r[14]
    ...
    with open(SEARCH_TRAINING_OUT_FILE, "w") as fp:
        all_examples = search_train_lcns + search_train_sns + search_train_dns
        if INCLUDE_CN_IN_FINAL_OUTPUT:
            all_examples.extend(search_train_cns)
        random.shuffle(all_examples)
        fp.writelines(ex[0] + "|" + ex[1] + "\n" for ex in all_examples)
    with open(RERANKER_TRAINING_OUT_FILE, "w") as fp:
        all_examples = reranker_train_lcns + reranker_train_sns + reranker_train_dns
        if INCLUDE_CN_IN_FINAL_OUTPUT:
            all_examples.extend(reranker_train_cns)
        random.shuffle(all_examples)
        fp.writelines(ex[0] + "|" + ex[1] + "\n" for ex in all_examples)
    with open(TESTING_OUT_FILE, "w") as fp:
        all_examples = validation_lcns + validation_sns + validation_dns
        if INCLUDE_CN_IN_FINAL_OUTPUT:
