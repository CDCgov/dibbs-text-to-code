...
def create_tsdae_data(nlp: spacy.Language, parts_fp: str, out_fp: str) -> None:
    """Constructs a collection of domain-adapted sentences fit for use with unsupervised TSDAE (Transformer-based Sentence-Denoising Auto-Encoder) pre-training.

    Applies a variety of preprocessing, formatting, and
    ...
    with open(parts_fp, encoding="utf-8") as fp:
        for loinc_line in fp:
            ...
            if stripped_loinc_line != "":
                if _line_starts_with_loinc_code(stripped_loinc_line) and curr_description != "":
...
def _line_is_citation(line: str) -> bool:
    """Helper method to determine if a given line of string text is an academic or institutional citation. A line is deemed a citation if it includes a text group of the form.
...
def _line_starts_with_loinc_code(line: str) -> Match | None:
    """Helper method to determine if a line of string text begins with a numerically formatted LOINC code (defined as a number of one or more digits, followed by a dash, and then one digit).
...
def _post_process_sentence(st: str) -> str:
    """Helper function that applies post-processing cleanups to the result of Spacy's sentence tokenizer.

    Some artifacts can't be caught in the
